...
func (ec *executionContext) resolveEntity(
	...
	switch typeName {
	case "Run":
		resolverName, err := entityResolverNameForRun(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Run": %w`, err)
		}
		switch resolverName {
		...
		case "findRunByID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findRunByID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindRunByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Run": %w`, err)
			}
...
func entityResolverNameForRun(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Run", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Run", ErrTypeNotFound))
			break
		}
		return "findRunByID", nil
	}
	return "", fmt.Errorf("%w for Run due to %v", ErrTypeNotFound,
